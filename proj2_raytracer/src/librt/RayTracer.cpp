...
void RayTracer::Run(Scene *pScene, std::string fName, RenderMode mode)
{
    ...
    STVector3 *lightDir = new STVector3(pScene->GetLightDirection());
    ...
    for (int x = 0; x < width; x++){
      for(int y = 0; y < height; y++){
        ...
        Shade = pShader->Run(intersec, lightDir);
        STImage::Pixel paint = STImage::Pixel(Shade.r,Shade.g,Shade.b,Shade.a);
        if (intCount > 0){
